import requests
import json
from vyper.vyper import Vyper as VyperCfg
...
class ExchangeClient(KalshiClient):
    def __init__(self, env: str, config: VyperCfg):
        super().__init__(
            config.get_string(f"{env}.exchangeAPIBase"),
            config.get_string(f"{env}.username"),
            config.get_string(f"{env}.password"),
        )
